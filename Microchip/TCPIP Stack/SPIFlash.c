...
BYTE FLASHReadyFlag, *FLASHBufferPtr, FLASHBuffer[FLASH_BUFFER_SIZE];
DWORD FLASHAddress;

/*********************************************************************
 * Function:        void SPITransfer(unsigned char inbyte)
 *
 * Input:           inbyte - byte to be sent
 *
 * Output:          None
 *
 * Side Effects:    None
 *
 * Overview:        Sends a single byte to the SPI buffer.
 ********************************************************************/
unsigned char SPITransfer(unsigned char inbyte)
{
	SSPBUF = inbyte;
	while(!PIR1_SSPIF);
	PIR1_SSPIF = 0;	
...
/*********************************************************************
 * Function:        void SPIFlashBeginRead(DWORD address)
 *
 * PreCondition:    SPIFlashInit() is already called.
 *
 * Input:           control - Flash control and address code.
 ...
 *                  Puts Flash in sequential read mode.
 *
 * Note:            This function does not release I2C bus.
 *                  User must call SPIFlashEndRead() when read is not longer
 *                  needed; I2C bus will released after SPIFlashEndRead()
 *                  is called.
 ********************************************************************/
void SPIFlashBeginRead(DWORD address)
{
...
/*********************************************************************
 * Function:        BYTE SPIFlashRead(void)
 *
 * PreCondition:    SPIFlashInit() && SPIFlashBeginRead() are already called.
 *
 * Input:           None
 *
 * Output:          The return from SPI byte.
 *
 * Side Effects:    None
 *
 * Overview:        Reads next byte from Flash; internal address
...
/*********************************************************************
 * Function:        void SPIFlashReadArray(DWORD address,
 *                                          BYTE *buffer,
 *                                          WORD length)
 *
 * PreCondition:    SPIFlashInit() is already called.
 ...
 * Note:            None
 ********************************************************************/
void SPIFlashReadArray(DWORD address,
                        BYTE *buffer,
                        WORD length)
{
	unsigned int addr	= 0;
	...
	// perform to transfer
	SPIFLASH_SPISTATbits.CKE = 0;	// Data transmitted on falling edge of SCK
...
/*********************************************************************
 * Function:        void SPIFlashBeginWrite(DWORD address)
 *
 * PreCondition:    SPIFlashInit() is already called.
 *
 * Input:           control     - data Flash control code
 ...
 * Side Effects:    None
 *
 * Overview:        Modifies internal address counter of Flash.
 *
 * Note:            This function is used only for writing
 *					 to the Flash.  Reads must use SPIFlashBeginRead(), 
 *					 SPIFlashRead(), and SPIFlashEndRead().
 *					This function does not release the SPI bus.
 ********************************************************************/
void SPIFlashBeginWrite(DWORD address)
{
	FLASHReadyFlag = 1;
	FLASHAddress = address;
	FLASHBufferPtr = FLASHBuffer;
...
/*****************************************************************************
  Function:
    void SPIFlashWriteArray(BYTE *val, WORD wLen)

  Summary:
    Writes an array of bytes to the Flash part.

  Description:
    This function writes an array of bytes to the Flash at the address 
    specified when SPIFlashBeginWrite() was called.  Page boundary crossing is 
    handled internally.
    
  Precondition:
    SPIFlashInit() was called once and SPIFlashBeginWrite() was called.
  ...
  Remarks:
    ...
    to call SPIFlashEndWrite() after calling this function.  However, if you do 
...
/*********************************************************************
 * Function:        void SPIFlashEndWrite(void)
 *
 * PreCondition:    SPIFlashInit() && SPIFlashBeginWrite() are already called.
 *
 * Input:           None
 *
 * Output:          SPIFlash_SUCCESS if successful
 *                  other value if failed.
 *
 * Side Effects:    None
 *
 * Overview:        Instructs Flash to begin write cycle.
 *
 ...
 *                  User must call for SPIFlashIsBusy() to ensure that write
...
/*********************************************************************
 * Function:        BOOL SPIFlashIsBusy(void)
 *
 * PreCondition:    SPIFlashInit() is already called.
 *
 * Input:           None
 *
 * Output:          FALSE if Flash is not busy
...
/*********************************************************************
 * Function:        void WriteBufferToMainMemory(DWORD address)
 *
 * PreCondition:    SPIFlashInit() is already called.
 *
...
/*********************************************************************
 * Function:        void ReadBufferFromMainMemory(DWORD address)
 *
 * PreCondition:    SPIFlashInit() is already called.
 *
...
/*********************************************************************
 * Function:        void ByteToBuffer(unsigned int byte, DWORD address)
 *
 * PreCondition:    SPIFlashInit() is already called.
 *
...
#endif //#if defined(USE_ATMEL_FLASH)

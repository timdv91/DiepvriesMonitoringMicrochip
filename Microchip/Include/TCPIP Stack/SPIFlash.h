...
#define SPI_FLASH_SECTOR_MASK		(SPI_FLASH_SECTOR_SIZE - 1)
	...
	#else
		#define	SPISelectFlash()	SPIFLASH_CS_IO = 0
		#define	SPIUnselectFlash()	SPIFLASH_CS_IO = 1
		#define	PIR1_SSPIF			SPIFLASH_SPI_IF

		unsigned char SPITransfer(unsigned char inbyte);
		void SPIFlashInit(void);
		void SPIFlashBeginRead(DWORD address);
		BYTE SPIFlashRead(void);
		void SPIFlashReadArray(DWORD address, BYTE *buffer, WORD length);

...
#define OUTPUT_RECEIVE_MAX_BYTES	4
#ifdef	WEBPAGE_DEMO_I2C
unsigned char I2C_String[OUTPUT_RECEIVE_MAX_BYTES*3]="";
#endif

#ifdef	WEBPAGE_DEMO_SPI
unsigned char SPI_String[OUTPUT_RECEIVE_MAX_BYTES*3]="";
#endif
...
// SPP +
	// Reset for bootloader option
	if(!memcmppgm2ram(filename, "index.htm", 9))
	{
		char *Temp;

		Temp = (char*)HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"Reset");
		if (Temp)
			Reset ();
	}

	// If its the UART Demos.htm page
	if(!memcmppgm2ram(filename, "UART Demo.htm", 13))
	{
		#ifdef	WEBPAGE_DEMO_UART
		char *Transmit, *BaudRate, *Temp;
		unsigned long int NewBaudRate=0;
		Temp = (char*)HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"Save");
		if (Temp)
		{
			int i;
			// code for BaudRate
			BaudRate = (char*)HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"BaudRate");
			for (i=0; BaudRate[i]; i++)
				NewBaudRate = NewBaudRate * 10 + (BaudRate[i] - '0');

			UART_Init (NewBaudRate);
		}

		Temp = (char*)HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"Send");
		if (Temp)
		{
			const char NewLine[] = "\n\r";
			UART_Write_String (NewLine);
			Transmit = (char*)HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"Command");
			UART_Write_String (Transmit);
		}
		#endif
	}

	// If its the I2C Demos.htm page
	if(!memcmppgm2ram(filename, "I2C Demo.htm", 12))
	{
		#ifdef	WEBPAGE_DEMO_I2C
		char *Temp, *I2C_Delay_String;
		int Address, Command;
		unsigned char Value[OUTPUT_RECEIVE_MAX_BYTES], Data, Old_OSCTUNE;
		...
		if (Temp)
		{
			int i;
			// code for I2C delay
			I2C_Delay_String = (char*)HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"I2C_Delay");
			I2C_Delay = 0;
			for (i=0; I2C_Delay_String[i]; i++)
				I2C_Delay = I2C_Delay * 10 + (I2C_Delay_String[i] - '0');
		}

		Temp = (char*)HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"Address");
		Address = Convert_String_to_Int (Temp);
		Temp = (char*)HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"Command");
		Command = Convert_String_to_Int (Temp);
		Temp = (char*)HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"Input");
		Data = Convert_String_to_Int (Temp);

		Temp = (char*)HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"Read");
		if (Temp)
		{
			if (Data>OUTPUT_RECEIVE_MAX_BYTES)
				Data = OUTPUT_RECEIVE_MAX_BYTES;
			I2C_Receive (Address, Command, Value, Data);
			ConvertArrayToString (Value, I2C_String, Data);
		}

		Temp = (char*)HTTPGetROMArg(curHTTP.data, (ROM BYTE *)"Write");
		if (Temp)
			I2C_Transmit (Address, Command, Data);
		OSCTUNE = Old_OSCTUNE;
		#endif
	}


	// If its the SPI Demos.htm page
	if(!memcmppgm2ram(filename, "SPI Demo.htm", 12))
	{
		#ifdef	WEBPAGE_DEMO_SPI
		char *Temp;
		unsigned char Value[OUTPUT_RECEIVE_MAX_BYTES], Data;
		long int i;
		...
		if (Temp)
		{
			if (Data>OUTPUT_RECEIVE_MAX_BYTES)
				Data = OUTPUT_RECEIVE_MAX_BYTES;
			DelayMs (200);
			SPI_Receive (Value, Data);
			ConvertArrayToString (Value, SPI_String, Data);
		}
...
{    
	...
	// R_ = R0 * e ^ (-B / T0), [ohm] --> const ~= 0.0106614
	// T = B/ln (r/R_), [K]
	#define	R0	((float)10000)
	#define	B	((float)4100)
	r = ((1023.0*R0)/(float)tmp)-R0;
	temperature = B/log(r/0.0106614) - 273.15;	// log is ln in this case
...
void HTTPPrint_UART_Output (void)
{
	#ifdef	WEBPAGE_DEMO_UART
	#define	CHUNK	0x80
	// Value of CHUNK defines the buffer size. So bigger number means better
	// performance but requires more stack memory. If you have problems with
	// the stack you can decrease this value.
	char Buff[CHUNK+1], i;

	if (RCIndex == TCIndex)
		return;

	for (i=0; i<CHUNK && RCIndex != TCIndex; i++, TCIndex++)
		if (UART_Buffer[TCIndex] == '<' || UART_Buffer[TCIndex] == '>')
			Buff[i] = UART_Buffer[TCIndex] + 31;	// <> replaced with []
		else
			Buff[i] = UART_Buffer[TCIndex];
	Buff[i]=0;
	TCPPutArray(sktHTTP, Buff, i);	// same as: TCPPutString(sktHTTP, Buff); but without need to calculate the length everytime
	#endif
	return;
}

void HTTPPrint_I2CRes (void)
{
	#ifdef	WEBPAGE_DEMO_I2C
	TCPPutString(sktHTTP, I2C_String);
	#endif
	return;
}

void HTTPPrint_SPIRes (void)
{
	#ifdef	WEBPAGE_DEMO_SPI
	TCPPutString(sktHTTP, SPI_String);
	#endif
	return;
}

...
static void ProcessIO(void)
{
    ...
    //vars needed for calculating average adc based on multiple readings: 
    int i = 0;
    int n=100;
    long int tot = 0;
    ...
    // ADC channel 9
	ADCON0bits.CHS0 = 1; //change this between 0 or 1 to switch between pot and temp.
	ADCON0bits.CHS1 = 1;
	ADCON0bits.CHS2 = 0;
	ADCON0bits.CHS3 = 1;
    
    //get average of 'n' measurements using for loop:
    for(i; i<n; i++)
    {
        //get adc value:
        ADCON0bits.GO = 1;
        ...
        while(ADCON0bits.GO);
        //add to long int, int is to small:
        tot += ADRES;
    }
    //calculate average using vars:
    ADRES = tot/n;
    ...
    #if !defined(__18F87J50) && !defined(_18F87J50) && !defined(__18F87J11) && !defined(_18F87J11) 
    {
        ...
        // 10bit to string conversion:
    uitoa(*((WORD*)(&ADRES)), AN0String);
    
    //10bit to temperature to string conversion:
    itoa(convertToDegrees((WORD)ADRES), AN1String);
#endif
}
...
int convertToDegrees(WORD adres)
{
    float arduADC,Vin;
    int temp, tmp;   
    ...
    */
    ...
    /*Calculate input voltage on ADC*/
    Vin = ((float)tmp/320);
    /*Calculate to arduino ADC equivalent:*/
    arduADC = Vin * 204.6f;
    /*Calculate temperature*/
    temp = (0.001*pow(arduADC,2))+(0.926*arduADC)+(-287.103);
    /*Fix offset:*/
    temp += fixTempOffsetUsingPotmeter();
    //temp -= 5;
    
    return (int)temp;//temperature;
    
    //itoa(temperature, AN0String);   
    //uitoa(*((WORD*)(&ADRES)), AN0String);
}

//fix offset on temperature reading using the onboard pottentio meter:
//each 100 ADC values = 2°C up or down. This gives a range of 10°C up or 10°C down.
int fixTempOffsetUsingPotmeter()
{
    int offset;
    // AN0 should already be set up as an analog input
    // ADC channel 9
	ADCON0bits.CHS0 = 0; //change this between 0 or 1 to switch between pot and temp.
	ADCON0bits.CHS1 = 0;
	ADCON0bits.CHS2 = 0;
	ADCON0bits.CHS3 = 1;
